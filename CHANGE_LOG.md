...
## [2026-07-03 22:44:08]
- **Type**: Fix (将平铺分屏左列改为内容驱动宽度)
- **Content**: 1) 将平铺模式左列从固定百分比改为目录内容驱动的 `fit-content` 列，右侧搜索与书签卡片区域占用剩余空间，避免不同屏幕比例下左侧空白被放大；2) 移除平铺模式搜索框旧的固定宽度与 `780px` 上限，让顶部搜索框与右侧内容列真实齐平；3) 为时间信息增加不换行保护，并让移动端覆盖重置左列最小宽度，确保窄屏继续上下堆叠。
- **Impact**: `styles.css`
