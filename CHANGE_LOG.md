...
- **Content**: 优化满屏状态下右侧卡片的拥挤感：1) 废弃强制的 4 列/3 列硬编码布局，改用 `repeat(auto-fill, minmax(220px, 1fr))` 纯自适应流式网格，杜绝卡片过度挤压；2) 水平间距从 16px 提升至 24px，垂直间距从 16px 提升至 20px，赋予卡片之间充足的呼吸空间；3) 基础行高从 96px 微调压缩至 86px，缩小卡片单体绝对体积，增加背景留白占比，彻底解决右侧“白板一面墙”的压抑感。
