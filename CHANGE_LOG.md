...
## [2026-05-14]
- **Time**: 2026-05-14 18:15:00 +0800
- **Type**: Refactor (Pure Mode UI Polish)
- **Content**: 全面重构纯净模式 UI，提升视觉高级感与交互体验。1) 容器深度毛玻璃：将 `backdrop-filter` 从 3px 大幅增至 20px，配合多层级柔和阴影与微弱内发光，打造通透悬浮感；2) 搜索框灵动化：由沉重的双行布局重构为单行紧凑布局，搜索框高度减小，聚焦时新增缩放位移反馈，视觉更加轻盈聚焦；3) 时钟排版优化：移除时钟组硬线条，微调间距与字体粗细对比，提升信息的呼吸感；4) 统一搜索引擎切换器样式，使其更自然地融入单行胶囊空间。
