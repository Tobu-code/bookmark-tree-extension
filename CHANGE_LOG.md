...
## [2026-07-14 14:13:00]
- **Type**: Hotfix (彻底解决 AI 侧边栏打开动画失效、页面瞬跳的问题)
- **Content**: 之前 `document.body.classList.add('ai-sidebar-open')` 位于 `requestAnimationFrame` 外部，导致侧栏移除了 `.hidden` (`display: none`) 之后瞬间被赋予了目标样式 `.ai-sidebar-open .ai-sidebar { transform: translateX(0) }`，从而绕过了 CSS 过渡动画。本次修复通过使用 `void sidebar.offsetWidth` 强制浏览器完成由于移除 `.hidden` 引发的 Reflow 布局重排，以确立初始的 `translateX(100%)` 离屏状态，然后将 `.ai-sidebar-open` 和 `.active` 类名的添加一起放入下一帧中，确保滑入动画完美执行。
- **Impact**: `src/08-ai-sidebar.js`
...
- **Impact**: `LAYOUT_ELEMENTS_GUIDE.md`

## [2026-07-14 10:11:00]
- **Type**: Fix (消除亮色自定义背景下强加的大半径白色文字发光，彻底修复字体粘黏发虚 Bug)
- **Content**: 在 `styles.css` 中重构亮色自定义背景（`has-custom-bg`）下的文字投影样式，移除了诗词标题、正文、署名、侧栏文件夹标题和书签卡片标题原本被强加的宽幅白色高斯发光阴影与毛边（`0 0 24px`/`12px`/`8px` 的 `rgba(255,255,255,...)`），使卡片标签与搜索输入框重归 `text-shadow: none` 极简锐利直出，诗词文本调整为 1-2 像素高对比暗色微投影，彻底恢复字体轮廓在亮色壁纸上的清晰可读性。
- **Impact**: `styles.css`
...
## [2026-07-13 23:17:00]
- **Type**: Design (调优相框模式底墙的虚实模糊度和高对比色彩呈现)
- **Content**: 在 `styles.css` 中，将环境氛围背景层 `#ambient-layer` 的高斯模糊半径由模糊过度、细节全失的 `60px` 调整为更富质感的 `16px`，保留壁纸的画面细节与大致轮廓；同时，将底墙在大自适应环境下的强力降曝机制舍弃，重新校准为温和降曝：深色模式亮度为 `0.8`，浅色模式为 `0.9`，从而打造相框背景与底板“一实一虚”的高级连贯式景深效果。
- **Impact**: `styles.css`
...
## [2026-07-13 21:45:00]
- **Type**: Design (调优自定义壁纸下自适应遮罩的透明度)
- **Content**: 在 `styles.css` 中，将深色自适应壁纸下的遮罩变量 `--custom-bg-scrim` 从偏暗的 `rgba(2, 6, 23, 0.48)` 下调至更加通透且温和的 `rgba(2, 6, 23, 0.22)`；同时将浅色自适应壁纸下的遮罩变量由 `rgba(255, 255, 255, 0.26)` 调至 `rgba(255, 255, 255, 0.15)`。这在保留原图色彩与亮度的同时，兼顾了桌面时间及文字的识别对比度。
- **Impact**: `styles.css`

## [2026-07-13 18:25:00]
- **Type**: Design (重构卡片毛玻璃特效，还原精致液态高光叠层材质)
- **Content**: 1) 在 `styles.css` 中移除尾部对 `.leaf-wrapper` 后置强行覆盖背景的 `!important` 纯色限制，还原原本多达四层的精细高光渐变（包含对角线反射高光与雅致左侧强调微光）；2) 恢复卡片伪元素 `::after` 的渲染逻辑，重现精致的玻璃高光折射边缘和强调色顶线；3) 调整卡片饱和度与对比度为 `saturate(145%) contrast(102%)`，解决背景压暗遮罩引发的浑浊感，使毛玻璃表现更加晶莹透亮；4) 微调卡片圆角到更为饱满优雅的 `16px`。
- **Impact**: `styles.css`
...
## [2026-07-10 18:20:00]
- **Type**: Fix (修复搜索建议面板与搜索引擎面板被裁剪消失问题)
- **Content**: 将 `.search-wrapper` 的 `overflow` 属性从 `hidden` 修改为 `visible`。绝对定位的搜索建议面板（#search-suggestions）和搜索引擎选择器（#engine-wheel）超出其边界，设置 overflow: hidden 会导致其无法显示。
- **Impact**: `styles.css`

## [2026-07-10 17:37:30]
- **Type**: Fix (优化时间格式、搜索框文字弱化与搜索框宽度)
- **Content**: 去除时间显示中的秒数，仅展示小时与分钟；移除搜索框输入文本的斜体样式，将占位符的字体粗细降至 400，并提升各背景主题下占位符的透明度以进行视觉弱化；调整搜索框的宽度为 8 列卡片等宽（计算公式为 8 个网格单元格宽度与 7 个间距之和）。
- **Impact**: `src/09-ui-widgets.js`, `styles.css`, `script.js`

## [2026-07-10 17:23:00]
- **Type**: Chore (加载外部技能目录)
- **Content**: 创建全局自定义插件 `user-agents-skills` 并编写配置，通过符号链接将 `/Users/zanehao/.agents/skills` 下的 21 个自定义技能映射至系统插件目录，使其可被 AI 助手直接加载和调用。
- **Impact**: `/Users/zanehao/.gemini/config/plugins/user-agents-skills` (系统外部配置)

## [2026-07-10 17:08:04]
- **Type**: Refactor (弱化首页辅助信息层级)
- **Content**: 进一步降低诗词、时间和日期的视觉对比；搜索引擎切换改为透明低强调文字按钮，仅在悬停或展开时显示背景，保持搜索输入为首要视觉动作。
- **Impact**: `styles.css`

## [2026-07-10 17:01:47]
- **Type**: Fix (调整首页顶栏对比、间距与搜索尺寸)
- **Content**: 降低诗词、时间与日期的文字对比；桌面端增大时间日期与搜索框间距，将搜索框缩短至原宽度约三分之二并降低至 54px 高度；移动端维持全宽。
- **Impact**: `styles.css`

## [2026-07-10 16:29:46]
- **Type**: Revert (撤回首页中心搜索轴)
- **Content**: 撤回桌面端首页诗词、时间与搜索的三列重排，恢复原有顶栏布局与搜索视觉。
- **Impact**: `styles.css`

## [2026-07-10 16:12:14]
- **Type**: Fix (修复 AI 列表裁切及搜索和时间视觉问题)
- **Content**: 移除 AI 设置滚动区与服务卡片的绘制隔离，修复后续服务行内容被裁切；关闭 AI 侧栏时不再重建主界面滤镜；搜索输入调整为 15px 斜体；时间脉搏取消缩放，避免日期产生视觉位移。
- **Impact**: `styles.css`

## [2026-07-10 16:01:12]
- **Type**: Fix (修复背景模糊、AI 顶栏与 GLM 图标)
- **Content**: 背景模糊滑块不再依赖自定义壁纸而被禁用；AI 服务切换栏改为不压缩的横向滚动工具栏；GLM 替换为智谱官网图标资源。
- **Impact**: `src/06-settings.js`, `src/01-constants.js`, `styles.css`, `icons/ai/glm.ico`

## [2026-07-10 15:47:40]
- **Type**: Fix (磨砂滑块接管书签卡片)
- **Content**: 书签卡片取消固定 18px 磨砂和固定透明度，改为使用设置面板输出的卡片透明度与模糊变量；滑块调整会立即同步卡片磨砂感。
- **Impact**: `styles.css`

## [2026-07-10 15:37:51]
- **Type**: Refactor (书签卡片切换为磨砂玻璃)
- **Content**: 将书签卡片从液态高光、扫光和缩放交互改为稳定的半透明磨砂表面，使用固定模糊、细边框和低饱和阴影；保留原有网格尺寸与图标布局。
- **Impact**: `styles.css`

## [2026-07-10 15:21:52]
- **Type**: Fix (移除书签列表外框并修正日期字形)
- **Content**: 日期改为正体；移除书签列表区域的装饰框线，列表容器保持透明、无边框和无阴影。
- **Impact**: `styles.css`

## [2026-07-10 14:32:20]
- **Type**: Revert (撤回纸面便当书签卡片)
- **Content**: 撤回未提交的书签卡片便当网格样式，恢复原有卡片视觉与交互表现。
- **Impact**: `styles.css`

## [2026-07-10 12:47:25]
- **Type**: Fix (调整目录侧栏视觉重心)
- **Content**: 为书签目录滚动区增加响应式左内边距，使桌面端目录内容向页面中心靠拢，移动端保持紧凑。
- **Impact**: `styles.css`

## [2026-07-10 12:43:27]
- **Type**: Fix (对齐诗词标题与正文中心线)
- **Content**: 诗词容器改为跟随最长诗句宽度，标题和作者信息使用同一宽度居中，修正标题相对正文偏移。
- **Impact**: `styles.css`

## [2026-07-10 11:57:37]
- **Type**: Fix (调整时间与诗词标题对齐)
- **Content**: 将时间数字固定为 42px；为诗词标题和作者信息补充居中对齐，确保标题在信息区内居中。
- **Impact**: `styles.css`

## [2026-07-10 11:53:10]
- **Type**: Fix (统一苹方字体与信息字号)
- **Content**: 全局字体统一为 PingFang SC 回退链；搜索输入、诗词正文、时间日期、诗词标题和时间数字调整为指定字号。
- **Impact**: `styles.css`

## [2026-07-10 11:48:00]
- **Type**: Fix (移除书签图标外壳并统一列表字号)
- **Content**: 取消书签与目录图标的固定外框、背景和圆角，保留原始 28px 图标；将左侧目录与右侧书签名称统一为 14px。
- **Impact**: `styles.css`
...
## [2026-07-07 18:21:21]
- **Type**: Fix (调整时间日期横排样式)
- **Content**: 将时间和日期恢复为一行横排，时间字号调整为日期的三倍并保留心跳动效；日期改为斜体，日期格式改为 `YYYY/MM/DD·星期X`，并收紧日期与星期之间的间隔。
- **Impact**: `styles.css`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-07 18:02:41]
- **Type**: Fix (调整时间日期错落对齐)
- **Content**: 将时间日期区改为两列两行网格，时间位于第一列第一行，日期位于第二列第二行，使时间右侧与日期左侧形成错落对齐；同步将时间心跳动效原点改到右侧，减少对齐漂移。
- **Impact**: `styles.css`

## [2026-07-07 17:59:05]
- **Type**: Fix (调整时间日期为上下布局)
- **Content**: 将时间日期区由横向排列改为上下两行，时间独占第一行并保留心跳动效，日期星期落到第二行；同步移除时间与日期之间的空格分隔节点，降低横向拥挤感并形成错落层级。
- **Impact**: `styles.css`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-07 17:52:20]
- **Type**: Fix (上移顶部信息并恢复秒级时间)
- **Content**: 将诗词、时间和搜索框所在的顶部 header 整体上移；时间显示从 `HH:mm` 改为 `HH:mm:ss` 并按秒刷新；将时间与日期之间的圆点分隔符替换为空格。
- **Impact**: `styles.css`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-07 17:43:21]
- **Type**: Fix (修复诗词与时间搜索区重叠)
- **Content**: 将诗词文字块宽度重新约束到左侧栏，移除自定义背景分支中的额外左右 padding，并收紧诗词标题、正文字号与字距，避免诗句横向侵入右侧时间和搜索区域。
- **Impact**: `styles.css`

## [2026-07-07 17:05:58]
- **Type**: Fix (重调诗词文字块与顶部节奏)
- **Content**: 将诗词区从毛玻璃卡片改为无容器书卷式文字块，移除背景、阴影、毛玻璃和分隔线；诗词字体改为宋体/衬线系并调整为标题、作者、正文的参考图式层级；收紧搜索区与书签卡片区之间的纵向间距，并将时间数字字号继续放大到日期的约 260%。
- **Impact**: `styles.css`
...
## [2026-07-06 18:19:35]
- **Type**: Fix (调整日期与时间右对齐)
- **Content**: 将顶部时间区日期行改为右对齐，使日期右边缘与上方时间右边缘保持一致，同时保留现有渐变流动与时间显示逻辑。
- **Impact**: `styles.css`

## [2026-07-06 18:15:47]
- **Type**: Fix (优化书签编辑动效开关样式)
- **Content**: 将编辑书签弹窗中的“心脏脉搏动效”开关调整为独立配置行，统一边框、浅底、圆角和内边距，并收敛开关胶囊尺寸与选中态样式，同时增加与底部操作按钮的间距。
- **Impact**: `styles.css`

## [2026-07-06 18:05:32]
- **Type**: Fix (修复滚动遮罩与毛玻璃控制)
- **Content**: 移除书签卡片独立滚动区上下渐隐遮罩，避免卡片边缘出现明显灰白横条；AI 侧边栏和顶栏改为跟随全局玻璃变量与主题色，不再写死深色背景；毛玻璃及卡片磨砂感设置不再依赖是否上传壁纸，并移除 AI 侧栏独立 blur 覆盖逻辑。
- **Impact**: `styles.css`, `src/06-settings.js`, `script.js`

## [2026-07-06 17:56:52]
- **Type**: Fix (书签卡片独立滚动区)
- **Content**: 将主页滚动职责从全局页面转移到左右内容区：顶部时间、标语和搜索框固定在首屏可见，右侧书签卡片使用独立滚动容器，左侧目录同步保持独立滚动，避免不同目录书签数量变化导致页面上下漂移。
- **Impact**: `styles.css`

## [2026-07-06 17:33:13]
- **Type**: Fix (优化时间区渐变与书签卡片字体)
- **Content**: 将时间和日期改为块级同起点左对齐，并为时间、日期复用标语的文字渐变流动效果；同步柔化书签卡片标题字重、颜色、行高和字体渲染，提升玻璃卡片下的精致感。
- **Impact**: `styles.css`

## [2026-07-06 17:08:38]
- **Type**: Fix (优化顶部信息区和 AI 侧栏关闭按钮)
- **Content**: 顶部日期、搜索框与标语整体下移 5vh；标语改为手写斜体风格并保留渐变流动；时间显示增加秒并按秒刷新，日期行与时间左对齐；增强 AI 侧栏顶部操作区层级与关闭按钮 pointer/click 捕获，改善右上角关闭难选中和点击无效问题。
- **Impact**: `newtab.html`, `styles.css`, `src/08-ai-sidebar.js`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-06 16:54:29]
- **Type**: Fix (修复设置弹窗 AI 服务列表加载遮挡)
- **Content**: 将设置弹窗的滚动职责从外层毛玻璃面板拆到内部设置内容区，并为 AI 服务列表和图标增加绘制隔离与固定渲染约束，避免图标加载或滚动时出现浅色矩形遮挡。
- **Impact**: `styles.css`

## [2026-07-06 16:33:08]
- **Type**: Fix (替换 AI 服务为真实本地图标)
- **Content**: 拉取并缓存预置 AI 服务的真实站点图标资源，顶部 AI 标签和设置页服务列表改为引用本地 `icons/ai/` 图标，避免使用字母徽标且继续保持离线可用；DeepSeek 保留已有内置 SVG。
- **Impact**: `icons/ai/`, `src/01-constants.js`, `script.js`
...
- **Impact**: `src/01-constants.js`, `src/06-settings.js`, `script.js`

## [2026-07-06 15:55:24]
- **Type**: Fix (对齐搜索框并扩大 AI 关闭按钮命中区)
- **Content**: 将右侧内容宽度改为按完整书签网格列计算，使搜索框右边缘与下方卡片实际最右侧对齐；同时扩大 AI 侧栏右上角操作按钮命中区并提升交互层级，改善关闭按钮难以选中或点击无效的问题。
- **Impact**: `styles.css`

## [2026-07-06 15:49:16]
- **Type**: Fix (修复书签卡片磨砂感设置不明显)
- **Content**: 降低自定义壁纸浅色分支下书签卡片的固定白底与高亮渐变强度，并让卡片毛玻璃明确绑定到磨砂变量，避免“毛玻璃及卡片磨砂感”滑块被过实卡片底色抵消。
- **Impact**: `styles.css`
...
## [2026-07-05 23:33:33]
- **Type**: Fix (彻底移除时间卡边框并收短搜索区)
- **Content**: 清理自定义背景浅色覆盖中重新赋给时间卡的边框颜色，改为单独保持透明边框和更轻阴影；同时为搜索区增加 `1280px` 宽度上限，使搜索框整体更收敛。
- **Impact**: `styles.css`

## [2026-07-05 23:28:44]
- **Type**: Fix (优化时间卡与搜索标语质感)
- **Content**: 移除左侧时间卡的可见边框，放大搜索框上方每日标语并提高字重与亮度，同时增加轻微呼吸动画和 reduced-motion 兜底，让顶部信息层级更自然。
- **Impact**: `styles.css`

## [2026-07-05 23:22:00]
- **Type**: Fix (移动每日标语到搜索框上方)
- **Content**: 将每日标语从左侧时间日期卡片中移出，改为显示在右侧搜索框上方；时间卡片仅保留时间和日期，搜索区获得更明确的情绪引导层级，同时保持原有标语生成逻辑不变。
- **Impact**: `newtab.html`, `styles.css`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-05 23:12:39]
- **Type**: Fix (增强平铺卡片呼吸感)
- **Content**: 在保持当前基础单位和默认 `2*1` 尺寸不变的前提下，将平铺卡片网格间距从 `11-15px` 提升到 `18-24px`，降低卡片密集感并恢复更舒适的视觉呼吸。
- **Impact**: `styles.css`

## [2026-07-05 22:59:17]
- **Type**: Fix (缩小卡片基础网格单位)
- **Content**: 将平铺卡片基础单位缩小为原来的 1/2，并同步收紧网格间距，使新的 `1*1` 等同于旧视觉尺度的 `0.5*0.5`，默认 `2*1` 成为更合理的基础长方形卡片。
- **Impact**: `styles.css`

## [2026-07-05 22:52:34]
- **Type**: Fix (修正 1*1 卡片基础单位为正方形)
- **Content**: 将平铺卡片网格的列宽和行高统一绑定到 `--flat-grid-cell`，并取消 `1fr` 拉伸列宽，确保 `1*1` 真正渲染为正方形，`2*1`/`1*2` 等尺寸按“长 × 宽”语义准确展开。
- **Impact**: `styles.css`

## [2026-07-05 22:44:52]
- **Type**: Fix (统一卡片布局尺寸语义)
- **Content**: 将卡片尺寸统一定义为“长 × 宽”，基础单位为 `1*1` 正方形，默认卡片改为 `2*1` 横向长方形；编辑弹窗仅暴露 `1*1` 到 `3*3` 的 9 种规范布局，并对历史存储中的非法尺寸增加回退保护。
- **Impact**: `newtab.html`, `src/02-bookmarks-core.js`, `src/03-bookmarks-items.js`, `script.js`

## [2026-07-05 22:24:06]
- **Type**: Fix (增强 AI 顶栏服务标签质感)
- **Content**: 适度放大 AI 顶栏服务标签的图标、文字、胶囊高度与横向间距，并加厚默认和激活态背景，让服务切换入口在沉浸式 AI 工作区中更清晰、更有实体感。
- **Impact**: `styles.css`

## [2026-07-05 22:13:36]
- **Type**: Fix (重塑 AI 侧栏为沉浸式工作区)
- **Content**: 将 AI 面板从 2/3 宽抽屉调整为更宽的沉浸式工作区，取消 iframe 虚拟画布缩放并恢复原生显示；同步弱化面板玻璃外壳、收紧顶部工具栏材质，并让左侧主页在 AI 打开时退为暗化背景上下文，减少三层侧边栏割裂感。
- **Impact**: `styles.css`

## [2026-07-05 22:05:15]
- **Type**: Fix (移除无效文件夹展开延迟并收紧 AI 设置区)
- **Content**: 删除设置页中已失效的“文件夹展开延迟”滑块及对应存储/监听逻辑，子文件夹悬浮展开改为固定低延迟；同时压缩 AI 服务管理列表的卡片高度、间距、图标和操作按钮尺寸，使其与其它设置项体量更一致。
- **Impact**: `newtab.html`, `styles.css`, `src/01-constants.js`, `src/03-bookmarks-items.js`, `src/06-settings.js`, `script.js`

## [2026-07-05 21:56:56]
- **Type**: Fix (收紧 AI 侧栏顶部服务标签)
- **Content**: 将 AI 侧栏顶部服务标签改为左对齐紧凑工具条，缩小标签高度、间距、文字与图标尺寸，并收紧右侧操作按钮，避免顶栏显得笨重且与嵌入页面左侧内容脱节。
- **Impact**: `styles.css`
...
## [2026-07-05 21:39:07]
- **Type**: Fix (修复搜索匹配结果框透明)
- **Content**: 将搜索建议面板提升为明确不透明的独立浮层，并为自定义壁纸场景和每条匹配结果补充实色承载背景，避免高透明度/复杂壁纸下结果框看不清。
- **Impact**: `styles.css`

## [2026-07-05 20:51:35]
- **Type**: Fix (优化搜索建议层级与 AI 侧栏宽度)
- **Content**: 1) 将搜索建议下拉改为不透明实色面板并提升层级，避免结果与书签卡片透明重叠导致看不清；2) 将 AI 侧栏改为右侧贴边全高面板，占用视口 2/3 宽度，并取消 iframe 缩放，提升内嵌页面完整展示效果。
- **Impact**: `styles.css`

## [2026-07-05 20:18:25]
- **Type**: Fix (修复搜索建议样式与 AI 侧栏内容裁切)
- **Content**: 1) 为搜索建议下拉列表补充 `#search-suggestions` scoped 子项样式，恢复结果项、图标、标题、URL 和目录标签的独立层级，避免结果内容覆盖书签卡片区域；2) 为 AI 侧栏 iframe 增加缩放画布，让内嵌 AI 页面以更完整的桌面内容比例展示，降低右侧内容被裁切的视觉问题。
- **Impact**: `styles.css`

## [2026-07-05 16:36:38]
- **Type**: Fix (修复浅色壁纸下目录未选中项发糊)
- **Content**: 移除浅色自定义壁纸场景下左侧目录项的白色文字阴影，并为未选中目录项增加轻量半透明承载底色与更高文字字重，提升复杂壁纸上的目录可读性。
- **Impact**: `styles.css`

## [2026-07-05 16:26:46]
- **Type**: Fix (提升浅色壁纸高磨砂下文字可读性)
- **Content**: 为浅色主题自定义壁纸场景增加专用文字、边框、玻璃面板和卡片承载层规则，避免高透明度/高毛玻璃设置下目录、搜索框、时间框与书签卡片文字被复杂壁纸吞掉。
- **Impact**: `styles.css`

## [2026-07-05 16:05:05]
- **Type**: Fix (修复壁纸层级与 AI 服务操作按钮换行)
- **Content**: 1) 自定义壁纸启用时清除 `body` 默认渐变背景，并将背景图、网格纹理和遮罩重新排序，避免上传壁纸被页面背景压在下层；2) 固定设置页 AI 服务列表右侧操作槽宽度，禁止“启用/默认”按钮文字换行，恢复同一行横向排列。
- **Impact**: `styles.css`

## [2026-07-05 14:44:21]
- **Type**: Refactor/Fix (删除树状模式并修复设置与壁纸回归)
- **Content**: 1) 删除树状模式渲染、布局切换入口、树状卡片拖拽模块和相关 CSS 残留，主页固定只渲染平铺模式；2) 隐藏并固定布局设置为平铺，避免历史存储中的 `tree` 继续进入旧代码路径；3) 为设置页 AI 服务列表补充 scoped 样式，修复全局卡片/图标规则污染导致图标变成白色圆点和列表结构丢失的问题；4) 为自定义壁纸增加 `has-custom-bg` 专用背景层与遮罩规则，确保上传后的壁纸在主页可见；5) 同步根目录运行文件 `script.js`，不编译 release 包。
- **Impact**: `newtab.html`, `styles.css`, `src/01-constants.js`, `src/02-bookmarks-core.js`, `src/03-bookmarks-items.js`, `src/04-drag-card.js`, `src/06-settings.js`, `src/07-drag-items.js`, `src/09-ui-widgets.js`, `script.js`

## [2026-07-05 13:08:28]
- **Type**: Fix (按桌面端修正默认卡片尺寸与目录滚动)
- **Content**: 1) 将平铺模式未手动配置的默认书签卡片尺寸改回 `1*1`，目录卡片默认保持 `2*1`；2) 缩小平铺、树状和目录中的图标尺寸，避免低清 favicon 被放大后模糊；3) 为卡片增加更明确的 soft-ui 多色顶部细光和 accent 覆层，让颜色适配不再只停留在背景；4) 将树状目录卡片内容区改为明确的内部滚动，恢复合理条目行高，避免大量书签重叠；5) 移除移动端重排适配，改为桌面端优先的最小宽度策略。
- **Impact**: `styles.css`, `src/03-bookmarks-items.js`, `script.js`

## [2026-07-05 12:51:49]
- **Type**: Fix (提升 Soft UI 精致感并修正交互细节)
- **Content**: 1) 取消平铺模式默认随机卡片尺寸，未手动配置的卡片统一使用 `2*1`，让卡片大小完全交给用户控制；2) 引入 Soft UI 资源中的 indigo、pink、mint、amber、orange、lavender 多色 token，用于背景光斑、卡片 accent、图标底色和目录图标状态；3) 收敛搜索框聚焦态，移除输入框内层粗边框，仅保留外层柔和 glow；4) 将 AI 侧栏桌面宽度调整为 2/3 屏，移动端保持全宽弹层；5) 增大树状卡片间距并压缩内部条目字号、行高与图标尺寸，降低列表重叠和拥挤感。
- **Impact**: `styles.css`, `src/03-bookmarks-items.js`, `script.js`

## [2026-07-05 12:34:48]
- **Type**: Fix (修复 Soft Masonry 卡片可读性与树状列表空白)
- **Content**: 1) 将平铺默认卡片尺寸进一步偏向 `2*1` 与中尺寸，减少小卡过多导致的标题缺失；2) 为平铺小卡补充专用图标、标题和内边距规则，确保标准卡片也显示书签名称；3) 缩小时间框体量、圆角和字号，使其从大面板回到顶部信息组件；4) 为树状模式目录卡片单独设置高度和跨行规则，避免内容被压成空白横线；5) 重写树状卡片内部书签条目样式，恢复“图标 + 标题”的可读列表结构。
- **Impact**: `styles.css`, `src/03-bookmarks-items.js`, `script.js`
...
- **Type**: Refactor (New Tab Layout Rhythm)
- **Content**: 继续优化顶部与书签区的视觉节奏：顶部时间/搜索区到内容区的垂直间距增大约 50%，内容区分割线拆为左侧目录段与右侧卡片段并保留中间截断，同时压缩搜索框内搜索引擎胶囊尺寸，降低其视觉占比。
- **Impact**: `styles.css`、`CHANGE_LOG.md`

## [2026-07-06]
- **Time**: 2026-07-06 13:54:41 +0800
- **Type**: Refactor (New Tab Layout Rhythm)
- **Content**: 调整顶部功能区与书签内容区的视觉节奏：增大时间/搜索区到目录和卡片区的垂直间距，在内容区上方增加淡色渐隐分割线，并统一顶部搜索列与右侧卡片列的水平间距，使搜索框左边界与书签卡片区域对齐。
- **Impact**: `styles.css`、`CHANGE_LOG.md`
