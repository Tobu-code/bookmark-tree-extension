...
## [2026-05-15 15:30:00]
- **Type**: UI/UX & Bug Fix
- **Content**: AI 侧边栏布局对齐与缩放适配。1) 宽度对齐：将 AI 侧栏宽度调整为 90% 并设置 1600px 上限，使其展开后与页面中心框完全对齐；2) 缩放适配：为 AI 页面引入 0.9 倍缩放，并同步扩大其虚拟视口（111%），解决了 AI 站点内部 UI（如对话框）被挤压消失的问题；3) 结构加固：通过 Flex 布局优化确保顶部选项卡在内容加载时不再消逝。
- **Impact**: `styles.css`
