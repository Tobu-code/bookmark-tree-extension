...
- **Content**: 响应用户需求，将左侧平铺模式的“书签目录”重构为“层级书签树列表”：1) 废弃原先展平的目录列表逻辑与独立的拖拽排序状态；2) 左侧使用递归 `renderFolderTree`，引入 Finder 侧边栏式折叠/展开与空文件夹显示，状态支持 `localStorage` 记忆；3) 更新样式 `.tree-folder-item` 与图标折叠按钮；4) 修改右侧视图逻辑，点击左侧目录时同时显示子目录卡片与子书签列表；5) 重写 Drag & Drop，左侧树与右侧网格现在直接复用 Case 3 排序逻辑，支持真实反向修改 Chrome 书签原生结构。
