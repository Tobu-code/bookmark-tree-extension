...
## [2026-05-14]
- **Time**: 2026-05-14 18:45:00 +0800
- **Type**: Refactor (Final UI Unification)
- **Content**: 深度统一纯净模式视觉风格，消除元素间的不一致感：1) AI 按钮重塑：移除星标，复刻 Claude 模型选择器风格，采用文本 + 下拉箭头（Chevron Down）的极简形式；2) 标签系统标准化：搜索引擎标签由全圆角改为 8px 矩形圆角，统一白底灰边视觉，去除彩色激活态，改用灰度差表现选中感；3) 图标质感优化：对标签内的 Favicon 进行去色处理（grayscale 0.2），使其与整体冷静、高级的界面氛围完美融合。
